...
void CDlgRomImage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_USE_IMAGE, m_CheckUseImage);
}
...
BEGIN_MESSAGE_MAP(CDlgRomImage, CDialog)
	...
	ON_BN_CLICKED(IDC_CHECK_USE_IMAGE, &CDlgRomImage::OnBnClickedCheckUseImage)
END_MESSAGE_MAP()
...
BOOL CDlgRomImage::OnInitDialog()
{
	CDialog::OnInitDialog();

	m_CheckUseImage.SetCheck(use_rom_image?TRUE:FALSE);

	return TRUE;
}

void CDlgRomImage::OnBnClickedCheckUseImage()
{
	use_rom_image=m_CheckUseImage.GetCheck()?true:false;
}
