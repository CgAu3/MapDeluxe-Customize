...
BOOL CDlgObjParam::OnInitDialog()
{
	...
	{
		if((i>>8)!=6 && (i>>8)!=7 && *CodeTran_Obj((i>>8)|((i&0xFF)<<8))!='?')
		{
			str.Format(_T("[%02X-%02X]%s"),i>>8,i&0xFF,CodeTran_Obj((i>>8)|((i&0xFF)<<8)));
			m_ComboClass.AddString(str);
			if(obj.class_id==((i>>8)|((i&0xFF)<<8)))m_ComboClass.SelectString(-1,str);
		}
	}
	ResetParam();
	...
	case 1:
		...
		m_EditP1.SetWindowText(str);
		str.Format(_T("%d"),obj.paramA&0xF);
		m_EditP2.SelectString(0,str);
		str.Format(_T("%d"),obj.paramB>>5);
		m_EditP3.SetWindowText(str);
		str.Format(_T("%d"),obj.paramB&0x1F);
		m_EditP4.SetWindowText(str);
		break;
	case 3:
		break;
	case 4:
...
void CDlgObjParam::ResetParam()
{
	...
	case 1:
		...
		m_StaticP2.SetWindowText(_T("行为编号"));
		m_EditP1.EnableWindow();
		m_EditP2.EnableWindow();
		m_EditP3.EnableWindow();
		m_EditP4.EnableWindow();
		m_CheckObjFloat.EnableWindow(FALSE);
		m_EditP2.ResetContent();
		for(int i=0;i<10;i++)
		{
			if(CodeTran_FoeBeh((u16)(ca|(cb<<8)),i))
			{
				str.Format(_T("%d:%s"),i,CodeTran_FoeBeh((u16)(ca|(cb<<8)),i));
				m_EditP2.InsertString(i,str);
			}
		}
		break;
	case 3:
		...
		break;
	case 4:
		m_StaticP1.SetWindowText(_T("缓存编号"));
		m_StaticP2.SetWindowText(_T(""));
		m_EditP1.EnableWindow();
		m_EditP2.EnableWindow(FALSE);
		m_EditP3.EnableWindow(FALSE);
