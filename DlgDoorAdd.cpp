...
void CDlgDoorAdd::OnXYMenu(UINT id)
{
	...
	u32 x,y;
	_stscanf_s(str,_T("(%d,%d)"),&x,&y);
	str.Format(_T("%d"),x);
	m_EditDoorP1.SetWindowText(str);
	str.Format(_T("%d"),y);
	m_EditDoorP2.SetWindowText(str);
}
...
void CDlgDoorAdd::UpdateClass()
{
	...
	switch(m_ComboDoorClass.FindString(-1,str))
	{
	case 0:
...
void CDlgDoorAdd::OnOK()
{
	...
	newdoor.door.to_step=_ttoi(str);
	m_EditDoorP1.GetWindowText(str);
	newdoor.door.x=_ttoi(str);
	m_EditDoorP2.GetWindowText(str);
	newdoor.door.y=_ttoi(str);
	m_EditDoorP3.GetWindowText(str);
	newdoor.door.to_x=_ttoi(str);
	m_EditDoorP4.GetWindowText(str);
	newdoor.door.to_y=_ttoi(str);
	newdoor.door.unk=0;
	CDialog::OnOK();
}
